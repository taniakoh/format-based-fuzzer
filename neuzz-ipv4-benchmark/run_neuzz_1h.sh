...
TARGET_SCRIPT="${ROOT_DIR}/neuzz_target.py"
...
if [[ ! -f "${TARGET_SCRIPT}" ]]; then
  echo "Neuzz target script not found at ${TARGET_SCRIPT}"
  exit 1
fi
...
export NEUZZ_TEXT_RESULTS_DIR="${RESULTS_DIR}"
export NEUZZ_TEXT_TIMEOUT_SECONDS="${NEUZZ_TEXT_TIMEOUT_SECONDS:-3}"
...
cleanup() {
  ...
  if [[ -n "${NEUZZ_PID:-}" ]]; then
    kill "${NEUZZ_PID}" >/dev/null 2>&1 || true
    wait "${NEUZZ_PID}" 2>/dev/null || true
  fi
  if [[ -n "${NN_PID:-}" ]]; then
...
"${PYTHON_BIN}" "${ROOT_DIR}/nn.py" "${PYTHON_BIN}" "${TARGET_SCRIPT}" > "${LOG_DIR}/nn.log" 2>&1 &
NN_PID=$!
...
EXPECTED_END_AT="$(date -Iseconds -d "+${DURATION_SECONDS} seconds")"
echo "Starting Neuzz for ${DURATION_SECONDS} seconds."
echo "started_at=${STARTED_AT}"
echo "expected_end_at=${EXPECTED_END_AT}"
echo "Live progress will refresh below. Raw logs are in ${LOG_DIR}/."

timeout "${DURATION_SECONDS}s" "${ROOT_DIR}/neuzz" -i "${INPUT_DIR}" -o seeds -l "${MAX_FILE_SIZE}" "${PYTHON_BIN}" "${TARGET_SCRIPT}" @@ > "${LOG_DIR}/neuzz.log" 2>&1 &
NEUZZ_PID=$!
