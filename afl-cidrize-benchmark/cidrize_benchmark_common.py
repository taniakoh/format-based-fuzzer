...
EXTRACTED_DIR = REPO_ROOT / "linux-cidrize-runner_extracted" / "PYZ.pyz_extracted"
if not EXTRACTED_DIR.exists():
    raise SystemExit(
        f"Expected extracted cidrize payload at {EXTRACTED_DIR}. "
        "Run pyinstxtractor against cidrize-runner-main/bin/linux-cidrize-runner first."
    )
...
try:
    ...
    raise SystemExit(
        "Failed to import the extracted cidrize payload. Run this benchmark from WSL "
        "with the same Python version family used for extraction."
    ) from exc
...
EXPECTED_INVALID_EXCEPTIONS = (AddrFormatError, ValueError, TypeError)
...
def analyze_cidrize_input(data: bytes) -> RunOutcome:
    text = data.decode("latin-1", errors="replace")
    oracle = evaluate_target_input("cidrize", text)
    ...
    if oracle.supported and oracle.expected_valid is False:
        return RunOutcome(parsed, RunBug("oracle_mismatch", "Parser accepted invalid cidrize input", None), oracle.reason)
...
def print_outcome_summary(input_text: str, outcome: RunOutcome) -> dict:
    ...
    print(f"Running cidrize function with the arguments: ipstr: {input_text} strict: False raise-errors: False")
