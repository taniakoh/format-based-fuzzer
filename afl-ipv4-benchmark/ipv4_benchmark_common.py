...
EXTRACTED_DIR = REPO_ROOT / "linux-ipv4-parser_extracted" / "PYZ.pyz_extracted"
if not EXTRACTED_DIR.exists():
    raise SystemExit(
        f"Expected extracted IPv4 payload at {EXTRACTED_DIR}. "
        "Run pyinstxtractor against ipv4ipv6/linux-ipv4-parser first."
    )
...
EXPECTED_INVALID_EXCEPTIONS = (pyparsing.ParseException, IPv4ParsingError, InvalidityBug)
...
def analyze_ipv4_input(data: bytes) -> RunOutcome:
    text = data.decode("latin-1", errors="replace")
    oracle = evaluate_target_input("ipv4", text)
    ...
    if oracle.supported and oracle.expected_valid is False:
        return RunOutcome(parsed, RunBug("oracle_mismatch", "Parser accepted invalid IPv4 input", None), oracle.reason)
...
def print_outcome_summary(input_text: str, outcome: RunOutcome) -> dict:
    ...
    print(f"Running the IPV4 parser with ipstr: {input_text}")
